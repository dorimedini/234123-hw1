...
#include <string.h>
#include <time.h>
#include "/usr/include/syscall_maxproc.h"
	...
	while (cin>>command) {
		...
		if (command=="kill") {
			...
			if (m.find(arg1)==m.end()) {
				cout<<line_num << ": " << "unknown process: "<<arg1<<endl;
				continue;
			}
			...
			if (zombies.find(arg1)==zombies.end()) {
				cout<<line_num << ": " << "unknown zombie process: "<<arg1<<endl;
				continue;
			}
			if (arg1=="main") {
				...
				cout<<line_num << ": " << "process already dead: "<<arg1<<endl;
			}
			zombies.erase(arg1);
			if (arg1!="main") 
				parent.erase(arg1);
			...
			if (m.find(arg2)!=m.end()) {
				cout<<line_num << ": " << "process "<<arg2<<" already exists"<<endl;
				continue;
			}
			...
			cout<<line_num << ": " << "process "<<arg1<<" has "<<shared->out.result<<" subprocesses"<<endl;
		} else if (command=="set") {
			int lim;
			cin>>arg1>>lim;
			if (m.find(arg1)==m.end()) {
			...
			if (shared->out.result>=0) {
				cout<<line_num << ": " << "process "<<arg1<<" has max_proc value of ";
				cout<<shared->out.result<<endl;
			}
		} else {
			cout<<line_num << ": " << "command "<<command<<" not found"<<endl;
			continue;
		}
		cout<< line_num << ": " << "errno: "<<strerror(shared->out.code)<<endl;
	}
	shared->exit=true;
	shared->in.ready=true;
}
