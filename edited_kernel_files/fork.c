	    ...
	    struct pt_regs *regs, unsigned long stack_size)
{
	 ...
	 * calls
	 */
	if (clone_flags & CLONE_PID) {				// Cloning with the same PID is only allowed
		if (current->pid)						// for the swapper process (which has a PID
			goto fork_out;						// of 0: current->pid == 0)
	}

	retval = -ENOMEM;							// If there was a memory allocation error,
	p = alloc_task_struct();					// return here
	if (!p)
		goto fork_out;
